...
public class TetraEventHandler {
    ...
    public static void onAttackEntityEvent(@NotNull AttackEntityEvent event){
        ...
        for(ItemStack hand : source.getHandSlots()) {
            ...
            double sourceLeech = ((ModularItem) hand.getItem()).getEffectLevel(hand, ItemEffect.get("source_leech"));
            if(sourceLeech > 0) {
                LazyOptional<IManaCap> manaCap = CapabilityRegistry.getMana(source);
            ...
            if(spellstriker > 0) {
                if(source.getCooldowns().isOnCooldown(hand.getItem())) return;
                ...
                // Deprecated changes into SpellCostCalcEvent
                ...
                SpellContext context = new SpellContext(level,spell, (LivingEntity) source, player, hand);
                SpellResolver resolver = new SpellResolver(context);

                resolver.onCastOnEntity(hand, target, InteractionHand.MAIN_HAND);

                source.getCooldowns().addCooldown(hand.getItem(), (int)Math.ceil(cooldown * 20));
            }
        }
    }
    ...
    public static void onProjectileImpactEvent(@NotNull ProjectileImpactEvent event){
        ...
        SpellContext context = new SpellContext(level,spell, (LivingEntity) owner, player, item);
        SpellResolver resolver = new SpellResolver(context);
    ...
    public static void onBlockAttack(@NotNull ShieldBlockEvent event){
        ...
        if(new Random().nextInt(101) <= (spellguardChance+1)) {
            ...
            SpellContext context = new SpellContext(level,spell, playerEntity, player, itemStack);
            SpellResolver resolver = new SpellResolver(context);
